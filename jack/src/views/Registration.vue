    ...
    v-col.auth__form(cols="12"   sm="6") 
      ...
      v-form.mb-4(ref='form' @submit.prevent="submit")
        ...
        .form__item(
          ...
          v-text-field(
            v-model='repeatPassword'   label='Repeat your password' required
            type="password" 
            :class="{ 'error': $v.repeatPassword.$error  }"
            @change="$v.repeatPassword.$touch()"
            )
          .error.Emailerror(v-if="!$v.repeatPassword.sameAsPassword") 
            v-icon(color="red" small left) mdi-alert-outline
        ...
        v-card.pa-3.mt-8(tile max-width="400" elevation="5")
          span Do You have an account ?
            router-link.pl-3(to="/login") Create an account
